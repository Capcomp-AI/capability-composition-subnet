...
```
capability_subnet/
├── common/          protocol contracts - constants, schemas, hashing, commitments, timelock, signing, traces
...
That claim cannot be made at present. A run's bodies are published two runs after they are submitted, and local mode needs the field of the run it is measuring, so it reads nothing and burns. It stays the default and stays described here because the design is unchanged and the gap is the publication delay rather than the mode; a validator running today should use endpoint mode.
