...
for label, row in sorted(rows.items(), key=lambda kv: -kv[1]["score"]):
    ...
    print(
        f"{label:34s} {row['score']:7.3f} [{lo:.3f}, {hi:.3f}] {row['output_tokens']:10,}  "
        f"{row['kind']}{mark}"
    )
...
for task in tasks:
    ...
    bs = max(
        ((k, v["per_task"][task]) for k, v in singles.items() if task in v["per_task"]),
        key=lambda kv: kv[1][0] / max(1, kv[1][1]),
        default=(None, [0, 0]),
    )
    bm = max(
        ((k, v["per_task"][task]) for k, v in merges.items() if task in v["per_task"]),
        key=lambda kv: kv[1][0] / max(1, kv[1][1]),
        default=(None, [0, 0]),
    )
