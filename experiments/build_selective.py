...
snap = load_snapshot()
source = SafetensorsAdapterSource(POOL)
...
for name, cfg in VARIANTS.items():
    r = Recipe(
        workflow_id=snap.registry.workflow_id,
        base_revision=snap.manifest.revision,
        source_snapshot_sha256=snap.sha256,
        selected_adapters=sorted(STRONG),
        merge=MergeSpec(
            combination_type=cfg["m"],
            density=cfg["d"],
            majority_sign_method=cfg["s"],
            random_seed=1000003,
        ),
    ...
    t = time.time()
    res = reconstruct(r, snap, source, output_dir=OUT / name, device=DEV)
    man[name] = {
        "path": str(OUT / name),
        "method": cfg["m"],
        "adapters": len(STRONG),
        "path_used": res.stats.merge_path,
        "artifact_sha256": res.artifact_sha256,
    }
    print(
        f"  {name:18s} {time.time() - t:6.1f}s  {res.stats.merge_path:12s} {len(STRONG)} adapters",
        flush=True,
    )
