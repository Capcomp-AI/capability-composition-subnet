...
The expensive thing in this system is the pinned 8B base model and its 333 MB
adapters. Almost nothing under test actually needs their size — the merge engine
cares about shapes and determinism, the comparator cares about numbers, the
engine loop cares about control flow. So the fixtures here build a miniature pool
with the same *structure* and a fraction of the volume, and the whole suite runs
on a laptop in seconds.

The few things that genuinely need the real pool are marked ``gpu`` and skipped
by default.
"""
...
#: Miniature base model. Four decoder layers so every layer group is populated,
#: and 16-wide projections so a merge is instant.
TINY_LAYERS = 4
TINY_WIDTH = 16
TINY_RANK = 8
...
def tiny_manifest() -> BaseManifest:
    ...
    Built directly rather than loaded, because the loader deliberately refuses a
    manifest whose depth disagrees with the compiled-in protocol constant — that
    check protects the real pool and would block this fixture.
    """
...
@pytest.fixture(scope="session")
def tiny_tensors(tiny_manifest: BaseManifest) -> dict[str, dict[str, torch.Tensor]]:
    """Deterministic weights for every test adapter.

    Session-scoped and seeded: two tests that both merge the same adapters must
    be merging identical inputs, or a determinism assertion would be testing the
...
def tiny_pool_dir(tmp_path_factory, tiny_tensors, tiny_manifest) -> Path:
    """The test adapters materialised on disk as safetensors."""
    import json
...
def store(tmp_path: Path):
    """A throwaway engine store."""
    from capability_subnet.backend.store import Store

    instance = Store(tmp_path / "engine.sqlite")
    try:
        yield instance
    finally:
        instance.close()
