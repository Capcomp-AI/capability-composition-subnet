...
def run_position(block, run_blocks, **kw):
    return _chain.run_position(block, run_blocks, **UNANCHORED, **kw)
...
def measuring_run_for(block, run_blocks, **kw):
    return _chain.measuring_run_for(block, run_blocks, **UNANCHORED, **kw)
...
class TestOneDefinitionOfWhichRunMeasuresACommitment:
    ...
    def test_it_agrees_with_the_predicate_at_every_point_in_a_run(self):
        ...
        for offset in (0, 1, 5_000, self.W // 2, self.W - 301, self.W - 300, self.W - 1):
            ...
            others = [r for r in range(410, 416) if r != run and measured_in_run(block, r, self.W)]
