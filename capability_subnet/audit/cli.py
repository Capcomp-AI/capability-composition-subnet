...
from capability_subnet.audit.verify import AuditResult, audit_run, verify_report
...
def _cmd_run(args: argparse.Namespace) -> int:
    ...
    try:
        if args.reports_dir:
            ...
            path = (
                f"/reports?run_id={args.run}&limit=200"
                if args.run is not None
            ...
            weights_path = f"/weights/{args.run}" if args.run is not None else "/weights"
            try:
                vector = WeightVector.model_validate(
                    _fetch(args.backend_url, weights_path, args.timeout)
                )
            except Exception:  # noqa: BLE001 - a run may predate any vector
                vector = None
...
def _cmd_replay(args: argparse.Namespace) -> int:
    """Regenerate a closed run's instances and re-score its published traces.
    ...
    try:
        if args.file:
            disclosure = RunDisclosure.model_validate_json(Path(args.file).read_text("utf-8"))
        else:
            disclosure = RunDisclosure.model_validate(
                _fetch(
                    args.backend_url,
                    f"/runs/{args.run}/disclosure",
                    args.timeout,
                )
            )
    except Exception as exc:  # noqa: BLE001
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run = subparsers.add_parser(
        "run", help="Audit a run's reports and the weight vector derived from them."
    )
    run.add_argument("--run", type=int, default=None)
    run.add_argument("--reports-dir", default=None, help="Audit local files instead.")
    run.add_argument("--weights", default=None, help="Local weight vector JSON.")
    ...
    replay = subparsers.add_parser(
        "replay",
        help="Re-score a closed run from its published disclosure. No GPU needed.",
    )
    replay.add_argument("--run", type=int, default=None)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "replay" and args.run is None and not args.file:
        return _report_error("pass --run or --file")
